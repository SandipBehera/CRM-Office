...
class PropertyController extends Controller
{
    ...
    public function newproperties(Request $req){
        ...
        if($req->isMethod('post')){
            ...
            $property_by=$data['pby'];
            $property_rera=$data['prera'];
            ...
            $new_property->property_by=$property_by;
            $new_property->property_rera=$property_rera;
            $new_property->pid=$property_id;
            ...
            foreach($data['size_type'] as $key =>$size){
                ...
                $property_details->property_price=$data['price'][$key];
    ...
    public function viewproperties(Request $req,$id){

        $amenities=amenities::all();

        return view('admin.property.viewproperties')->with(compact('amenities'));
    }
    public function editproperties(Request $req,$id){
        if($req->isMethod('post')){
            $data=$req->all();
            $property_name=$data['pname'];
            $property_by=$data['pby'];
            ...
            $property_id=$data['pid'];
            $property_type=$data['propety_type'];
            $property_status=$data['propety_status'];
            $property_desc=$data['desc'];
            $metakeywords=$data['metakeywords'];
            $metadesc=$data['metadesc'];
            $project_location=$data['plocation'];
            ...
            $size_type=$data['size_type'];
            $property_size=$data['size'];
            Properties::where('pid',$id)->update(['property_name'=>$property_name,'property_by'=>$property_by,'property_rera'=>$property_rera,'pid'=>$property_id,'property_type'=>$property_type,'property_status'=>$property_status,'description'=>$property_desc,'meta_description'=>$metadesc,'meta_keywords'=>$metakeywords,'location'=>$project_location,'city'=>$project_city,'state'=>$project_state,'pincode'=>$project_pincode]);
            //Properties_details::where(['pid'=>$id)->update(['property_type'=>]);
            //floor Images
            $floor_image=$req->file('fimage');
            $property_image=$req->file('pimage');
        ...
        $get_property=Properties::where('pid',$id)->first();
        $get_property_details=Properties_details::where(['pid'=>$get_property->pid])->get();
        $get_property_details_image=property_image::where(['pid'=>$get_property->pid])->where('property_image','!=',null)->get();
        $get_property_floor_image=property_image::where(['pid'=>$get_property->pid])->where('floor_image','!=',null)->get();
        $amenities=amenities::all();
        return view('admin.property.editproperty')->with(compact('amenities','get_property','get_property_details','get_property_details_image','get_property_floor_image'));
    }
    public function deleteproperty(Request $req){
        $id=$req->id;
        Properties::where('pid',$id)->delete();
        Properties_details::where('pid',$id)->delete();
        property_image::where('pid',$id)->delete();
        return response()->json();
    }
    public function deletefloor(Request $req){
        $id=$req->image_id;
        property_image::where('floor_image',$id)->delete();
        return response()->json();
    }
    public function deletepropimage(Request $req){
        $id=$req->prop_image_id;
        property_image::where('property_image',$id)->delete();
        return response()->json();
    }
}
